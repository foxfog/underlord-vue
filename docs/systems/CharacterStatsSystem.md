...
> 3. Названия `skill_points` и `spell_points` (а также `skill_points_per_level` и `spell_points_per_level`) сохраняются полными, чтобы исключить двусмысленность и путаницу между очками спелов (`spell_points`) и маной (`mp` / `mpmax`).
> 
