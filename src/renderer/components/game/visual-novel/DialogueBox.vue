		...
		if (newVal) {
			nextTick(() => {
				if (dialogueTextRef.value) {
					...
					const isNewDialogue =
						newVal !== lastDialogueValue && props.multiStepPrintedLength === 0
					...
					if (isNewDialogue) {
						...
						console.log(
							'🖤 [Watch] Multi-step continuation, keeping existing text. multiStepPrintedLength:',
							props.multiStepPrintedLength
						)
					} else {
						console.log(
							'🖤 [Watch] Dialogue changed but conditions unclear. isNew:',
							isNewDialogue,
							'printed:',
							props.multiStepPrintedLength
						)
