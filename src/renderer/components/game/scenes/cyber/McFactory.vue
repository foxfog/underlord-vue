...
<script setup>
	...
	const bgImage = computed(() => {
		return props.scene?.bg ? `${basePath.value}${props.scene.bg}` : `${basePath.value}images/sprites/backgrounds/factory.webp`
	})
...
<style scoped>
	...
	.factory-smoke-layer {
		...
		width: 100%;
		height: 100%;
		background: radial-gradient(ellipse at center top, rgba(100, 100, 100, 0.1) 0%, transparent 70%);
		opacity: 0.6;
	}
	...
	.factory-lighting-overlay {
		...
		width: 100%;
		height: 100%;
		background: linear-gradient(180deg, rgba(200, 150, 100, 0.05) 0%, rgba(100, 100, 100, 0.1) 100%);
		opacity: 0.5;
	}
</style>
