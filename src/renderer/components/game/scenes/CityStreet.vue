<template>
	<div class="scene-city-street">
		...
		<div v-if="hasMod('fog')" class="scene-modifier scene-modifier-fog">
			<img :src="fogImagePath" alt="Fog" class="fog-layer fog-layer-1" />
			<img :src="fogImagePath" alt="Fog" class="fog-layer fog-layer-2" />
		</div>
	</div>
</template>
...
const hasMod = (modName) => {
	return (
		props.scene?.mods && Array.isArray(props.scene.mods) && props.scene.mods.includes(modName)
	)
}
</script>
